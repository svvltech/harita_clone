...
export const addImprovedPlane = async (): Promise<void> => {
  try {
    ...
    const position = Cesium.Cartesian3.fromDegrees(29.02, 41.02, 0);
    ...
    const baseWidth = img.width/10;   // örn: 100px → 100m
    const baseHeight = img.height/10; // örn: 100px → 100m
    ...
    // BoundingSphere: Model'in updateBoundingSphere'de oluşturduğu yapı
    /**
     * Cesium'a "bu kürenin merkezi şu koordinatta, yarıçapı 42.4m" bilgisini verir.
     * Cesium bu küreyi ekrana project ederek piksel-metre oranını hesaplar. Bu fonksiyon otomatik olarak:
        Perspektif kamerayı destekler (uzaktakiler küçük)
        Ortografik kamerayı destekler (uzaklık boyutu etkilemez)
        Kameranın FOV'unu hesaba katar
     */
    const boundingSphere = new Cesium.BoundingSphere(position, initialRadius);
