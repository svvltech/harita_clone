...
export const runImageToGlbStressTest = async (count: number = 100): Promise<void> => {
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const lon = 28.0 + Math.random() * 4.0;
            const lat = 38.0 + Math.random() * 4.0;
            ...
            // Yeni GLB oluştur (Her döngüde yeni bir Blob URL oluşur)
            const modelUri = await createTexturedQuadGlb('./icons/airplane_blue.png', 50);
