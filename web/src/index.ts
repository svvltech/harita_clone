...
import {
  ...
  ucusRotasiEkle2_2,
  ucusRotasiEkle2_ilk,
  ucusRotasiEkle2_kusursuz,
  ucusRotasiEkle_anchor,
  ucusRotasiEkle_arrowMesh,
  ucusRotasiEkleIlk,
  ucusRotasiEkleIlk_Border,
  ucusRotasiEkle_hybrid,
} from "./polyline_work/ucusRotasi";

Ion.defaultAccessToken =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIzYzczNWQwYy1iNmE4LTRiYWYtOTlmNi02OGFlOTM4YmUyYmEiLCJpZCI6MjY3NzM2LCJpYXQiOjE3NDQyMzE2ODN9.U72KcRtBEhdciByjNEOXCdG2T_rogCDdwziOsx4b6yg";
    ...
    async (
      ...
      if (!entityStats[id]) {
        ...
        if (delta > 0) {
          ...
          if (entityStats[id].samples.length > 5)
            entityStats[id].samples.shift();
          entityStats[id].hz =
            entityStats[id].samples.reduce((a, b) => a + b, 0) /
            entityStats[id].samples.length;
        }
...
function startHttp() {
  ...
  httpInterval = window.setInterval(() => {
    ...
    connection
      .invoke("RequestHttp", "https://httpbin.org/delay/2", ++msgId)
      .catch((err) => {
...
function startPing() {
  ...
  pingInterval = window.setInterval(() => {
    metrics.pingsSent++;
    connection
      .invoke("SendPing", metrics.pingsSent)
...
function startData() {
  ...
  dataInterval = window.setInterval(() => {
    ...
    connection
      .invoke("RequestHeavyData", config.dataSizeKB, ++msgId)
      .catch((err) => {
...
function updateMetrics() {
  ...
  if (!el) return;
  const elapsed =
    metrics.startTime > 0 ? (performance.now() - metrics.startTime) / 1000 : 0;
...
function updateEntityStatsUI() {
  ...
  if (!el) {
    ...
    el.style.cssText =
      "position:fixed;top:20px;left:20px;background:rgba(0,0,0,0.7);color:#fff;padding:12px;border-radius:8px;font-family:monospace;font-size:13px;z-index:9999;border:1px solid rgba(255,255,255,0.2);backdrop-filter:blur(4px);";
...
function createUI() {
  ...
  metricsEl.style.cssText =
    "position:fixed;top:10px;right:10px;background:rgba(0,0,0,.9);color:#0f0;padding:15px;border-radius:10px;font-family:Consolas,monospace;font-size:12px;z-index:10000;min-width:180px;border:1px solid #0f0";
  document.body.appendChild(metricsEl);

  // Control panel
  const panel = document.createElement("div");
  panel.style.cssText =
    "position:fixed;bottom:20px;left:20px;background:rgba(20,20,40,.95);padding:20px;border-radius:12px;z-index:10000;font-family:Segoe UI,sans-serif;max-width:300px";
  panel.innerHTML = `
        <style>
  ...
  // Config inputs
  document
    .getElementById("cfgMaxConcurrent")
  ...
  // Test buttons
  document
    .getElementById("btnHttp")
    ?.addEventListener("click", () =>
      httpInterval ? stopHttp() : startHttp(),
    );
  document
    .getElementById("btnPing")
    ?.addEventListener("click", () =>
      pingInterval ? stopPing() : startPing(),
    );
  document
    .getElementById("btnData")
    ?.addEventListener("click", () =>
      dataInterval ? stopData() : startData(),
    );
...
async function initializeCesium() {
  ...
  try {
    ...
    try {
      ...
      console.warn("modelManager yüklenemedi:", e);
    }
    ...
    try {
      ...
      console.warn("planeManager yüklenemedi:", e);
    }
    ...
    try {
      ...
      console.warn("imageToGlbManager yüklenemedi:", e);
    }
    ...
    try {
      ...
      console.warn("improvedPlaneYedek yüklenemedi:", e);
    }
    /*
        try {
...
*/
    ...
    try {
      const {
        ...
        ucusRotasiEkle1,
        ucusRotasiEkle2,
        ucusRotasiEkle_corridor,
        ucusRotasiEkle_sabit,
      } = await import("./polyline_work/ucusRotasi");
      //await ucusRotasiEkle_anchor();
      //await ucusRotasiEkle1();

      //await ucusRotasiEkle2();
      // await ucusRotasiEkle2_2(); // bundan devam edeceğiz
      // await ucusRotasiEkle2_ilk();
      ...
      //await ucusRotasiEkle_hybrid();
      await ucusRotasiEkle();
      
    } catch (e) {
      console.warn("ucusRotasiEkle yüklenemedi:", e);
    }
    ...
    document.addEventListener("keydown", async (e) => {
      if (e.key === "r" || e.key === "R") {
        try {
          const { interaktifRotaCiziminiBaslat } =
            await import("./polyline_work/ucusRotasi");
          interaktifRotaCiziminiBaslat(1000.0);
          console.log(
            "✏️ Çizim aracı aktif! Haritaya SOL TIK ile noktalar ekleyin. SAĞ TIK ile bitirin. (İrtifa: 1000m)",
          );
        } catch (err) {
          console.error("Çizim aracı başlatılamadı:", err);
        }
      }
    });
