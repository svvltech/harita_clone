...
import {
  ArrowEdgeMaterialProperty,
  ArrowEdgeMaterialProperty1,
  ArrowEdgeMaterialProperty2,
  ArrowEdgeMaterialProperty2duzgun_1,
  ArrowEdgeMaterialProperty2duzgun_2,
  ArrowEdgeMaterialProperty2duzgun_3,
  ArrowEdgeMaterialProperty2duzgun_4,
  ArrowEdgeMaterialProperty2son,
  ArrowEdgeMaterialProperty_anchor,
  ArrowEdgeMaterialProperty_Border_Ekle,
  ArrowEdgeMaterialProperty_Border_Ekle_v2,
  ArrowEdgeMaterialProperty_Border_Ekle_v3,
  ArrowEdgeMaterialProperty_glsl,
  ArrowEdgeMaterialProperty_Kusursuz,
  ArrowEdgeMaterialPropertyIlk,
  ArrowEdgeMaterialPropertyIlk_Border,
  ...
  Uclu_dongu_ChevronArrowEdgeMaterialProperty_kesik_serit_mesafeli,
  Dongu_ChevronArrowEdgeMaterialProperty_kesik_serit_mesafeli,
  ChevronArrowEdgeMaterialProperty_sandwichLine_tr
} from "./shaders";
...
export const ucusRotasiEkle1 = (): void => {
  try {
    ...
    const routeStartPoint = Cesium.Cartesian3.fromDegrees(
      28.8144,
      40.9769,
      1000.0,
    );
    ...
    const rotaKoordinatlari = Cesium.Cartesian3.fromDegreesArrayHeights([
      28.8144,
      40.9769,
      1000.0, // Kalkış sonrası tırmanış
      29.0,
      41.0,
      3500.0, // Ara irtifa
      29.5,
      40.8,
      8000.0, // Seyir irtifasına yaklaşım
...
export const ucusRotasiEkle2 = (): void => {
  ...
  for (let i = 0; i < rotaKoordinatlari.length - 1; i++) {
    toplamMetre += Cesium.Cartesian3.distance(
      rotaKoordinatlari[i],
      rotaKoordinatlari[i + 1],
    );
  }
...
export const ucusRotasiEkle2_ilk = (): void => {
  if (!viewer) return;
  ...
  const materyal = new ArrowEdgeMaterialProperty(Cesium.Color.WHITE, dashColor);
...
export const ucusRotasiEkle_sabit = (): void => {
  ...
  viewer.zoomTo(viewer.entities);
  console.log(
    `✅ Sabit ok rotası eklendi. Uzunluk: ${(toplamMetre / 1000).toFixed(1)} km, Ok sayısı: ${Math.round(toplamMetre / (toplamMetre / 10))}`,
  );
};
...
export const ucusRotasiEkle2son = (): void => {
  try {
    ...
    const rotaKoordinatlari = Cesium.Cartesian3.fromDegreesArrayHeights([
      28.8144,
      40.9769 + kayma,
      10000.0, // 1. Başlangıç noktası
      29.1144,
      40.9769 + kayma,
      10000.0, // 2. Dönüş BAŞLANGICI (0.3 boylam gidildi)
      29.6144,
      40.5 + kayma,
      10000.0, // 3. Dönüş BİTİŞİ (Çapraz inildi)
      29.9144,
      40.5 + kayma,
      10000.0, // 4. Bitiş noktası (Yine tam 0.3 boylam gidildi)
    ]);
    // Çizginin koordinat dizisi elinde var: rotaKoordinatlari
    let gercekUzunlukMetre = 0;
    for (let i = 0; i < rotaKoordinatlari.length - 1; i++) {
      gercekUzunlukMetre += Cesium.Cartesian3.distance(
        rotaKoordinatlari[i],
        rotaKoordinatlari[i + 1],
      );
    }
    const ortaNokta =
      rotaKoordinatlari[Math.floor(rotaKoordinatlari.length / 2)];
...
export const ucusRotasiEkleIlk = (): void => {
  try {
    ...
    // 3. Custom materyalimizi örnekliyoruz
    const ucusRotasiMateryali = new ArrowEdgeMaterialPropertyIlk(
      okRengi,
      çizgiRengi,
    );
...
export const ucusRotasiEkleIlk_Border = (): void => {
  try {
    ...
    // 3. Custom materyalimizi örnekliyoruz
    const ucusRotasiMateryali = new ArrowEdgeMaterialProperty_Border_Ekle_v3(
      okRengi,
      çizgiRengi,
      borderColor,
      borderWidth,
...
export const ucusRotasiEkle = (): void => {
  try {
    ...
    new ChevronArrowEdgeMaterialProperty_sandwichLine_tr(okRengi, çizgiRengi);
...
export const ucusRotasiEkle_anchor = (): void => {
  try {
    ...
    const ucusRotasiMateryali = new ArrowEdgeMaterialProperty_anchor(
      okRengi,
      çizgiRengi,
      viewer.scene,
      rotaKoordinatlari[0],
...
function rotaBoyuncaOrnekle(
  ...
  for (let i = 0; i < pozisyonlar.length - 1; i++) {
    ...
    const yon = Cesium.Cartesian3.subtract(
      bitis,
      baslangic,
    ...
    while (sonrakiOrnekMesafe <= toplamMesafe + segUzunluk) {
      ...
      const poz = Cesium.Cartesian3.lerp(
        baslangic,
        bitis,
        t,
        new Cesium.Cartesian3(),
...
function okModelMatrisiHesapla(
  ...
  const tersEnu = Cesium.Matrix4.inverseTransformation(
    enuMatris,
    new Cesium.Matrix4(),
  );
  const yonEnu = Cesium.Matrix4.multiplyByPointAsVector(
    tersEnu,
    yon,
    new Cesium.Cartesian3(),
  );
...
export function rotaOklariniOlustur(
  ...
  const pozlar = new Float64Array([
    -yarimL,
    -govdeYarimW,
    zOfset, // 0: gövde sol-arka
    govdeBitis,
    -govdeYarimW,
    zOfset, // 1: gövde sol-ön
    govdeBitis,
    govdeYarimW,
    zOfset, // 2: gövde sağ-ön
    -yarimL,
    govdeYarimW,
    zOfset, // 3: gövde sağ-arka
    govdeBitis,
    -basYarimW,
    zOfset, // 4: baş sol
    yarimL,
...
export const ucusRotasiEkle_arrowMesh = (): void => {
  ...
  viewer.zoomTo(viewer.entities);
  console.log(
    `✅ Arrow Mesh eklendi. Uzunluk: ${(toplamMetre / 1000).toFixed(1)} km, Ok sayısı: ${ornekSayisi(rotaKoordinatlari, toplamMetre / 15)}`,
  );
};
...
function ornekSayisi(poz: Cesium.Cartesian3[], aralik: number): number {
  ...
  for (let i = 0; i < poz.length - 1; i++)
    toplam += Cesium.Cartesian3.distance(poz[i], poz[i + 1]);
  ...
  cizimDinleyici.setInputAction(
    (click: Cesium.ScreenSpaceEventHandler.PositionedEvent) => {
      ...
      if (sabitNoktalar.length < 2) {
        console.warn(
          "Rota çizimi iptal edildi. En az 2 nokta çizilmiş olmalı.",
        );
        return;
      }
      ...
      console.log(
        "İnteraktif rota başarılıyla çizildi. " +
          sabitNoktalar.length +
          " noktalı.",
      );
    },
...
// ============================================================================
// HİBRİT MATERYAL TEST FONKSİYONU
// ============================================================================
// Bu fonksiyon, ArrowEdgeMaterialPropertyHybrid sınıfını test etmek için
// haritaya Z şeklinde 3 segmentli bir Polyline ekler.
//
// Hibrit materyalin çalışma prensibi:
//   - Ok KONUMU: UV (materialInput.st.s) ile belirlenir → Pan'de kaymaz
//   - Ok ŞEKLİ:  gl_FragCoord ile çizilir → Perspektiften bağımsız
//   - Ok SAYISI:  CPU her karede repeatCount hesaplar → Zoom'da dinamik
//
// Test senaryoları:
//   1. Haritayı sağa-sola kaydır (pan) → Oklar çizgiye yapışık kalmalı
//   2. Kamerayı eğ (tilt)              → Ok şekilleri bozulmamalı
//   3. Zoom in/out yap                 → Ok sayısı artmalı/azalmalı
//   4. Farklı açılardan bak            → Oklar eşit aralıklı kalmalı
// ============================================================================
export const ucusRotasiEkle_hybrid = (): void => {
  if (!viewer) return;

  // Diğer test rotalarından ayırt etmek için enlem kaydırması
  const kayma = 0.2;

  // Z şeklinde 3 segmentli rota koordinatları (Boylam, Enlem, İrtifa-Metre)
  // Segment 1: Doğuya düz (29.0→29.5, sabit enlem)
  // Segment 2: Güneydoğuya çapraz (29.5→29.75, enlem düşüyor)
  // Segment 3: Doğuya düz (29.75→30.25, sabit enlem)
  const rotaKoordinatlari = Cesium.Cartesian3.fromDegreesArrayHeights([
    29.0,
    41.0 + kayma,
    10000.0,
  ...
  // Rotanın toplam metre uzunluğunu hesapla
  // Bu değer CPU'da repeatCount hesaplaması için gerekli
  let toplamMetre = 0;
  for (let i = 0; i < rotaKoordinatlari.length - 1; i++) {
    toplamMetre += Cesium.Cartesian3.distance(
      rotaKoordinatlari[i],
  ...
  // Dash (arka plan çizgisi) rengi: Mor/pembe tonu
  const dashColor = new Cesium.CallbackProperty(() => {
    return Cesium.Color.fromBytes(239, 12, 249, 255);
  }, false);

  // Hibrit materyali oluştur
  // Parametreler:
  //   arrowColor: WHITE → Okların rengi beyaz
  //   dashColor: Mor → Arka plan çizgisinin rengi
  //   scene: Kamera bilgilerine erişim için
  //   toplamMetre: Rotanın gerçek dünya uzunluğu
  //   dashPixels: 115 → Bir hücredeki çizgi boşluğunun piksel uzunluğu
  //   arrowPixels: 35 → Bir hücredeki ok şeklinin piksel uzunluğu
  //   Toplam desen = 150px, ok oranı = 35/150 ≈ %23
  const materyal = new ArrowEdgeMaterialPropertyHybrid(
    Cesium.Color.WHITE,
    dashColor,
    viewer.scene,
    toplamMetre,
  ...
  // Polyline entity'sini haritaya ekle
  viewer.entities.add({
    name: "Hybrid Arrow Rotasi",
    polyline: {
      positions: rotaKoordinatlari,
      width: 14,                         // Çizgi kalınlığı (piksel)
      material: materyal,                // Hibrit ok materyali
      clampToGround: false,              // Havada kalsın (10km irtifa)
      arcType: Cesium.ArcType.NONE,      // Segmentler arası büküm olmasın
    },
  });
